...
import requests
import logging
from config import TOKEN_TG, TOKEN_NEWS_API, TOKEN_FMP, TOKEN_BYBIT, TOKEN_OKX, TOKEN_ALPHA_VANTAGE, TOKEN_IEX_CLOUD, TOKEN_POLYGON
import time
...
def get_stock_info(symbol):
    ...
    if alpha_vantage_data:
        response_text += f"\n2. Data from Alpha Vantage:\n"
        response_text += f"   Current Price: ${alpha_vantage_data['price']:.2f}\n"
        response_text += f"   Change: ${alpha_vantage_data['change']:.2f} ({alpha_vantage_data['change_percent']:.2f}%)\n"
    else:
        response_text += "\nData from Alpha Vantage is not available.\n"

    if iex_data:
        response_text += f"\n3. Data from IEX Cloud:\n"
        response_text += f"   Current Price: ${iex_data['price']:.2f}\n"
        response_text += f"   Change: ${iex_data['change']:.2f} ({iex_data['change_percent']:.2f}%)\n"
    else:
        response_text += "\nData from IEX Cloud is not available.\n"

    if polygon_data:
        response_text += f"\n4. Data from Polygon.io:\n"
        response_text += f"   Current Price: ${polygon_data['price']:.2f}\n"
        response_text += f"   Change: ${polygon_data['change']:.2f} ({polygon_data['change_percent']:.2f}%)\n"
    else:
        response_text += "\nData from Polygon.io is not available.\n"
...
# Add new functions for the new services
def get_alpha_vantage_data(symbol):
    url = f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={symbol}&apikey={TOKEN_ALPHA_VANTAGE}"
    try:
        response = requests.get(url)
        response.raise_for_status()
...
def get_iex_data(symbol):
    url = f"https://cloud.iexapis.com/stable/stock/{symbol}/quote?token={TOKEN_IEX_CLOUD}"
    try:
        response = requests.get(url)
        response.raise_for_status()
        data = response.json()
        ...
        logger.error(f"Error fetching data from IEX Cloud for {symbol}: {e}")
        return None

def get_polygon_data(symbol):
    url = f"https://api.polygon.io/v2/aggs/ticker/{symbol}/prev?apiKey={TOKEN_POLYGON}"
    try:
        response = requests.get(url)
        response.raise_for_status()
        ...
        logger.error(f"Error fetching data from Polygon.io for {symbol}: {e}")
        return None
